...
class index {
	...
	public function show() {
		...
		if($CATEGORYS[$catid]['parentid']!=0){
			$cur_catid = $CATEGORYS[$catid]['parentid'];
		}else{
			$cur_catid = $catid;
		}
	...
	public function lists() {
		...
		if($cur_catid  == 7 || $catid == 7){
			$get_all_level = $this->get_all_level();
		}
		if($type==0) {
	...
	protected function _category_priv($catid) {
	  ...
	  */
	 public function get_zizhi_type(){
	 	$category_db = pc_base::load_model('category_model');
		$result = $category_db->select(array('parentid'=>7));
		$return = array();
		foreach ($result as $key => $value) {
			$return[$value['catid']] = $value['catname'];
		}
		$parent_id = isset($_GET['parent_id']) ? intval($_GET['parent_id']) : -1;
		if($parent_id > 7){
			$content_list  = $_content_list = array();
			$content_db = pc_base::load_model('content_model');
			$content_db->set_model(12);
			$content_list = $content_db->select(array('catid'=>$parent_id),'*','100');
			if(!empty($content_list))
			foreach ($content_list as $key => $value) {
				$_content_list[$value['id']] = $value['title'];
			}
			exit(json_encode($_content_list));
		}else{
	  ...
	  * @return [type] [description]
	  */
	public function get_zizhi_level(){
		$catid = intval($_GET['parent_id']);
		$id = intval($_GET['tid']);

		if(!$catid || !$id) exit(false);

		$siteids = getcache('category_content','commons');
		$siteid = $siteids[$catid];
		$CATEGORYS = getcache('category_content_'.$siteid,'commons');

		if(!isset($CATEGORYS[$catid]) || $CATEGORYS[$catid]['type']!=0)  exit(false);
		$this->category = $CAT = $CATEGORYS[$catid];
		$this->category_setting = $CAT['setting'] = string2array($this->category['setting']);
		$siteid = $GLOBALS['siteid'] = $CAT['siteid'];
		...
		$tablename = $this->db->table_name = $this->db->db_tablepre.$MODEL[$modelid]['tablename'];
		$r = $this->db->get_one(array('id'=>$id));
		if(!$r || $r['status'] != 99)  exit(false);
		$level = string2array($r['level']);
		$_res = array();
		if(is_array($level)){
			foreach ($level as $key => $value) {
				$_res[$key] = $value['level'];
			}
		}
	 ...
	 * @return [type] [description]
	 */
	private function get_all_level(){
		$this->db->set_model(12);
		$content_list = $this->db->select('','*','100');
		$CATEGORYS = getcache('category_content_1','commons');
		$CAT = $CATEGORYS[7]['arrchildid'];
		$catlist = explode(',',$CAT);
		$return = array();
		foreach ($content_list as $value) {
			$level = string2array($value['level']);
			foreach ($level as $key => $val) {
				$levels[] = array($key=>$val["level"]);
			}
			$return[$value['id']] = $levels;
			$levels = false;
		}
