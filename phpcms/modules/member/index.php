...
class index extends foreground {
	...
	public function register() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$userinfo['username'] = $username = (isset($_POST['username']) && is_username($_POST['username'])) ? $_POST['username'] : exit('0');
			$userinfo['nickname'] = (isset($_POST['nickname']) && is_username($_POST['nickname'])) ? $_POST['nickname'] : $username ;
			...
			$userinfo['email'] = $username.'@lian-qi.com';
			$userinfo['regip'] = ip();
			...
			//取用户手机号
			$mobile_verify = $_POST['verifyCode'] ? intval($_POST['verifyCode']) : '';
			if($mobile_verify=='') showmessage('请提供正确的手机验证码！', HTTP_REFERER);
			$sms_report_db = pc_base::load_model('sms_report_model');
			$posttime = SYS_TIME-360;
			$where = "`id_code`='$mobile_verify' AND `mobile`='$username' AND `posttime`>'$posttime'";
			$r = $sms_report_db->get_one($where,'*','id DESC');
			if(!empty($r)){
				exit(json_encode(array('result'=>false,'msg'=>array('verifyCode'=>'验证码错误'))));
			}
			...
			if(pc_base::load_config('system', 'phpsso')) {
				...
				exit(json_encode(array('result'=>false,'msg'=>array('verifyCode'=>'注册失败,该账号已存在'))));
			}
			exit(json_encode(array('result'=>false,'msg'=>array('verifyCode'=>'注册失败'))));
	...
	public function login() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$username = isset($_POST['username']) && is_username($_POST['username']) ? trim($_POST['username']) : exit(json_encode(array('result'=>false,'msg'=>array('phone'=>'手机号码不能为空'))));
			$password = isset($_POST['password']) && trim($_POST['password']) ? trim($_POST['password']) :exit(json_encode(array('result'=>false,'msg'=>array('phone'=>'密码不能为空'))));
			is_password($_POST['password']) && is_badword($_POST['password'])==false ? trim($_POST['password']) : exit(json_encode(array('result'=>false,'msg'=>array('phone'=>'密码格式不对'))));
			...
			if(pc_base::load_config('system', 'phpsso')) {
				...
				if(isset($memberinfo['uid'])) {
					...
					if($status == -1) {	//用户不存在
						exit(json_encode(array('result'=>false,'msg'=>array('phone'=>'用户不存在'))));
						showmessage(L('user_not_exist'), 'index.php?m=member&c=index&a=login');
					} elseif($status == -2) { //密码错误
						exit(json_encode(array('result'=>false,'msg'=>array('password'=>'密码错误'))));
						showmessage(L('password_error'), 'index.php?m=member&c=index&a=login');
					} else {
						exit(json_encode(array('result'=>false,'msg'=>array('phone'=>'登入失败'))));
				...
				if($rtime['times'] > 4) {
					...
					exit(json_encode(array('result'=>false,'msg'=>array('password'=>L('wait_1_hour')))));
				...
				if(!$r)exit(json_encode(array('result'=>false,'msg'=>array('phone'=>'用户不存在'))));
			...
			//如果用户被锁定
			if($r['islock']) {
				exit(json_encode(array('result'=>false,'msg'=>array('password'=>'用户被锁定'))));
			...
			exit(json_encode(array('result'=>true,'url'=>$forward)));
